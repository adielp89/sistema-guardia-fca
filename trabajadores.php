...
if (@$hoy[weekday] == $semana[4]){
    ...
    for($i=1;$i<$dias;$i++)
    {
        ...
        $resulta = $model->recolectaDatosT($dia);
        foreach ($resulta as $key => $value) {
			...
			$Turno=$value['Turno'];
			//echo $Nombre."===>".$email."===>".$Departamento."===>".$Turno."===>".$dia." <br>";
			...
			$mail->setFrom('usuario@uclv.edu.cu', 'Mailer');
...
// EN CASO DE NO SER FIN DE SEMANA--DE LUNES A JUEVES.
		...
		$resulta = $model->recolectaDatosT($diamas);
        foreach ($resulta as $key => $value) {
			//print_r($value);
			$Nombre = $value['Nombre'];
			$email = $value['email'];
			$Departamento= $value['Departamento'];
			$Turno=$value['Turno'];
			echo $Nombre."===>".$email."===>".$Departamento."===>".$Turno."===>".$diamas." <br>";
			...
			$mail->setFrom('usuario@uclv.edu.cu');
